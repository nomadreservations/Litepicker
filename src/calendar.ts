...
export class Calendar {
    ...
    protected calendars: DateTimeParser[] = [];
    protected picker: HTMLElement;
    protected datePicked: DateTimeParser[] = [];
    ...
    protected renderMonth(date: DateTimeParser) {
        ...
        if (this.options.dropdowns.months) {
            ...
            for (let x = 0; x < 12; x += 1) {
                ...
                const optionMonth = new DateTimeParser(
                ...
                option.disabled =
                    (this.options.minDate &&
                        optionMonth.isBefore(
                            new DateTimeParser(this.options.minDate),
                            "month"
                        )) ||
                    (this.options.maxDate &&
                        optionMonth.isAfter(
                            new DateTimeParser(this.options.maxDate),
                            "month"
                        ));
        ...
        if (this.options.dropdowns.years) {
            ...
            for (let x = maxYear; x >= minYear; x -= 1) {
                const option = document.createElement("option");
                const optionYear = new DateTimeParser(
                    new Date(x, 0, 1, 0, 0, 0)
                );
                option.value = x;
                option.text = x;
                option.disabled =
    ...
    protected renderDay(date: DateTimeParser) {
        date.setHours();
        ...
        if (
            this.options.minDate &&
            date.isBefore(new DateTimeParser(this.options.minDate))
        ) {
        ...
        if (
            this.options.maxDate &&
            date.isAfter(new DateTimeParser(this.options.maxDate))
        ) {
            day.classList.add(style.isLocked);
        }
        ...
        if (this.options.minDays && this.datePicked.length === 1) {
            ...
            const left = this.datePicked[0]
                .clone()
                .subtract(this.options.minDays - hotelMode, "day");
            const right = this.datePicked[0]
                .clone()
                .add(this.options.minDays - hotelMode, "day");
        ...
        if (this.options.maxDays && this.datePicked.length === 1) {
            ...
            const left = this.datePicked[0]
                .clone()
                .subtract(this.options.maxDays + hotelMode, "day");
            const right = this.datePicked[0]
                .clone()
                .add(this.options.maxDays + hotelMode, "day");
