...
export class Litepicker extends Calendar {
    ...
    constructor(options) {
        ...
        if (this.options.lockDays.length) {
            this.options.lockDays = DateTimeParser.convertArray(
        ...
        if (this.options.bookedDays.length) {
            this.options.bookedDays = DateTimeParser.convertArray(
        ...
        if (this.options.highlightedDays.length) {
            this.options.highlightedDays = DateTimeParser.convertArray(
        ...
        if (
            ...
            !("disallowBookedDaysInRange" in options)
        ) {
        ...
        if (
            ...
            !(this.options.startDate instanceof DateTimeParser)
        ) {
        ...
        if (
            !this.options.singleMode &&
            (!(this.options.startDate instanceof DateTimeParser) ||
                !(this.options.endDate instanceof DateTimeParser))
        ) {
            this.options.startDate = null;
                ...
                const afterOrientationChange = () => {
                    if (isMobile() && this.isShowning()) {
                        ...
                        if (!this.options.inlineMode) {
                            ...
                            this.picker.style.top = `calc(50% - ${
                                pickerBCR.height / 2
                            }px)`;
                            this.picker.style.left = `calc(50% - ${
                                pickerBCR.width / 2
                            }px)`;
                        }
                    }
    ...
    private parseInput() {
        ...
        if (this.options.elementEnd) {
            ...
            const d1 = splittedValue
                .slice(0, splittedValue.length / 2)
                .join(delimiter);
            const d2 = splittedValue
    ...
    private updateInput() {
        ...
        if (this.options.singleMode && this.options.startDate) {
            ...
            const startValue = this.options.startDate.format(
                this.options.format,
                this.options.lang
            );
            const endValue = this.options.endDate.format(
                this.options.format,
                this.options.lang
            );
    ...
    private onClick(e) {
        ...
        if (target.classList.contains(style.dayItem)) {
            ...
            this.datePicked[this.datePicked.length] = new DateTimeParser(
                target.dataset.time
            );
            ...
            if (this.shouldCheckBookedDays()) {
                ...
                const booked = this.options.bookedDays.filter((d) => {
                    if (d instanceof Array) {
                        return (
                            d[0].isBetween(
                                this.datePicked[0],
    ...
    private showTooltip(element, text) {
        const tooltip = this.picker.querySelector(
    ...
    private onMouseEnter(event) {
        ...
        if (typeof this.options.onDayHover === "function") {
            this.options.onDayHover.call(
                this,
                DateTimeParser.parseDateTimeParser(target.dataset.time),
        ...
        if (this.shouldAllowMouseEnter(target)) {
            ...
            let date2 = new DateTimeParser(target.dataset.time);
            let isFlipped = false;
            ...
            const allDayItems = Array.prototype.slice.call(
                this.picker.querySelectorAll(`.${style.dayItem}`)
            );
            allDayItems.forEach((d: HTMLElement) => {
                const date = new DateTimeParser(d.dataset.time);
    ...
    private onInput(event) {
        ...
        if (isValid) {
            ...
            this.options.startDate = new DateTimeParser(
                startValue,
                this.options.format,
                this.options.lang
            );
            ...
            if (endValue) {
                this.options.endDate = new DateTimeParser(
                    endValue,
                    this.options.format,
                    this.options.lang
                );
